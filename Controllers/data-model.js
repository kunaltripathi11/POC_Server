...
// add data model
exports.addDataModel =
    (async (req, res) => {
        const insertQuery = 'INSERT INTO data_model (name,description,app_package,primary_key_field,query, created_by_id, updated_by_id) VALUES ($1,$2,$3,$4,$5,$6,$7)'
        const { name, description, app_package, primary_key_field, query, updated_by_id } = req.body
        const created_by_id = userID
        try {
            const result = await pool.query(insertQuery, [name, description || null, app_package || null, primary_key_field || null, query, created_by_id, updated_by_id || null])
            console.log(result);
...
exports.updateDataModel = async (req, res) => {
    ...
    const updateQuery = "UPDATE data_model SET name=$1, description=$2, app_package=$3,primary_key_field=$4,query=$5,  updated_by_id=$6, updated_at=NOW() WHERE id=$7 "
    const { name, description, app_package, primary_key_field, query, } = req.body
    try {
        const results = await pool.query(updateQuery, [name, description || null, app_package || null, primary_key_field || null, query, updated_by_id || null, id])
