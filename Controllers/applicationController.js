...
exports.getApplication = async (req, res) => {
	const getAppQuery =
		"Select a.*, c.category_name,d.url,s.title as solution_title FROM solution_category s right join category c on s.id=c.sol_category_id  RIGHT JOIN application a  ON a.category_id=c.id LEFT JOIN dashboard d ON d.app_id=a.id WHERE a.is_deleted=FALSE ORDER BY a.display_order,a.title";
	try {
...
exports.deleteApplication = async (req, res) => {
	const deleteAppQuery =
		"UPDATE application SET is_deleted=true, updated_at=NOW(), updated_by_id=$1 WHERE uuid=$2 returning *";
	const updated_by_id = userID;
	try {
		const result = await pool.query(deleteAppQuery, [
			updated_by_id,
			req.params.id,
		]);
		res.status(200).json({
