    ...
    try {
        ...
        for (let tag of tags) {
            ...
            if (check.rows.length > 0) {
                ...
                const addToTagQuery = "INSERT INTO tags (tag) VALUES ($1) RETURNING id";
                const result = await pool.query(addToTagQuery, [tag]);
                tagId = result.rows[0].id;
            }

            const checkRelationQuery = "SELECT * FROM tag_rules WHERE tag_id=$1 AND rule_id=$2";
            const existingRelation = await pool.query(checkRelationQuery, [tagId, business_rule_id]);

            if (existingRelation.rows.length === 0) {
                const addToTagsRulesTable = "INSERT INTO tag_rules (tag_id, rule_id) VALUES ($1, $2)";
                await pool.query(addToTagsRulesTable, [tagId, business_rule_id]);
            }

            addedTags.push(tag);
        }

        res.status(201).json({ message: `Tags added successfully: ${addedTags.join(', ')}` });
    } catch (error) {
        console.error(error);
...
exports.getTag = async (req, res) => {
    ...
    const getTagQuery = "SELECT tag from tags WHERE tag LIKE $1 WHERE is_deleted=FALSE"
    try {
